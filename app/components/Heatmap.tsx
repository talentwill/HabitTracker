...
export default function Heatmap(props: {
  ...
  const weeks = useMemo(() => {
    ...
    for (const e of props.events) {
      ...
      if (
        !existing ||
        (newAction && priority(newAction) > priority(existing.action || ''))
      ) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2 gap-2">
        <div className="text-[14px] sm:text-[13px] font-semibold text-ink shrink-0">
          打卡热力图
        </div>
