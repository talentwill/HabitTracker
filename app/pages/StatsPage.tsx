...
export default function StatsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [sRes, hRes] = await Promise.all([api.summary(), api.listHabits({ archived: false })]);
      setSummary(sRes);
      ...
      const statsPromises = hRes.habits.map(async (h) => {
        ...
        const totalIntervals = Math.max(1, Math.ceil((totalDays + 1) / h.intervalDays));
        const completionRate =
          totalIntervals > 0 ? Math.round((doneCount / totalIntervals) * 100) : 0;
        ...
        for (let i = 0; i < doneEvents.length; i++) {
          if (i === 0) {
            ...
            const gap = diffDays(doneEvents[i - 1]!.actionDate, doneEvents[i]!.actionDate);
  ...
  const avgCompletionRate = useMemo(() => {
    ...
    return Math.round(
      habitStatsList.reduce((s, h) => s + h.completionRate, 0) / habitStatsList.length
    );
  }, [habitStatsList]);
  ...
  const rankedHabits = useMemo(() => {
    return [...habitStatsList].sort((a, b) => b.completionRate - a.completionRate);
  }, [habitStatsList]);
    ...
    <div className="pb-16 sm:pb-0 max-w-[800px] mx-auto">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Habit Ranking */}
          <div className="mb-5">
            <div className="text-[16px] font-semibold text-ink mb-2">习惯排行</div>
            <div className="rounded-lg border border-line bg-paper overflow-hidden">
              {rankedHabits.length === 0 ? (
                <div className="px-3.5 py-3 text-[14px] text-muted">暂无数据</div>
              ) : (
          ...
          {tagStats.length > 0 && (
            <div className="mb-5">
              <div className="text-[16px] font-semibold text-ink mb-2">标签统计</div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {tagStats.map((t) => {
                  const c = tagColor(t.tag);
                  return (
                    <div key={t.tag} className="rounded-lg border border-line bg-paper px-3 py-2.5">
          ...
          {/* Recent Events */}
          <div>
            <div className="text-[16px] font-semibold text-ink mb-2">最近动态</div>
            <div className="rounded-lg border border-line bg-paper overflow-hidden">
              {sortedRecentEvents.length ? (
                  ...
                  return (
                    <div
                      ...
                      className={clsx(
                        ...
                        i < sortedRecentEvents.length - 1 && "border-b border-line"
                      )}
                    >
                      <span
                        className={clsx(
                      ...
                      >
                        {e.action === "done" ? "打卡" : e.action === "push" ? "推迟" : "跳过"}
                      </span>
                      <span className="font-medium text-ink">{e.habit.title}</span>
                ...
                <div className="px-3.5 py-3 text-[14px] text-muted">暂无记录</div>
              )}
            </div>
          </div>
        </>
