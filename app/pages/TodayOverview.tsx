...
import { statusForDue, todayDateOnly, upcomingGroup, UPCOMING_GROUP_ORDER } from "../lib/date";
...
export default function TodayOverview() {
  ...
  const groupedHabits = useMemo(() => {
    if (filterTab === "todo") {
      ...
      if (todayList.length > 0) groups.push({ label: "今日到期", habits: todayList });
      if (overdueList.length > 0) groups.push({ label: "已逾期", habits: overdueList });
      return groups;
    }
      ...
      <div ref={containerRef} className="flex h-[calc(100vh-28px)] sm:h-[calc(100vh-32px)]">
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 py-2 border-b border-[rgba(180,160,200,0.15)]">
            <span className="text-[18px] sm:text-[16px] font-bold text-ink">📋 今日概览</span>
