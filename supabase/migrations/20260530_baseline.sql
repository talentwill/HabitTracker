-- Baseline migration: capture current remote schema as of 2026-05-30
-- This replaces all previous migrations and represents the full database state.
...
CREATE UNIQUE INDEX IF NOT EXISTS profiles_api_key_key ON profiles(api_key) WHERE api_key IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_habits_user_due_date ON habits(user_id, next_due_date);
...
CREATE POLICY "Users can CRUD own habit events"
...
BEGIN
  ...
  IF v_habit.start_date IS NOT NULL AND p_action_date < v_habit.start_date THEN
  ...
  INSERT INTO habit_events (user_id, habit_id, action, action_date, from_due_date, to_due_date)
  VALUES (auth.uid(), p_habit_id, 'done', p_action_date, v_habit.next_due_date, v_new_due);

  IF v_new_due > v_habit.next_due_date THEN
    UPDATE habits SET next_due_date = v_new_due, updated_at = now()
    WHERE id = p_habit_id;
  END IF;
